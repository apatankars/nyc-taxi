...
function prettify(key) {
  const overrides = {
    ...
    uses_udf: "IRIS-side arm",
    rate_code: "Rate code",
    mean_gap: "Mean gap",
    off_tariff: "Off tariff",
    pct_off_tariff: "% off tariff",
  };
 ...
 *
 * A series colour may be a string or (row, i) => colour, for the case where one
 * series key spans bars belonging to different entities.
 *
    ...
    rows.forEach((row, i) => {
      ...
      series.forEach((s, j) => {
        ...
        const mark = svg("path", {
          ...
          // A series may colour by row -- same entity, same hue, across a set of
          // bars that share one series key. Must happen here, inside draw(), so
          // it survives the resize redraws.
          fill: typeof s.color === "function" ? s.color(row, i) : s.color,
        });
        marks.push(mark);
        root.append(mark);
...
/**
 * The headline figures as one table rather than a row of stat cards. Six
 * bordered boxes spend most of their ink on borders and read as six unrelated
 * things; in a table the figures share one baseline and one set of rules, so
 * they read as one set of numbers about one file.
 *
 * One figure per *column*: the labels are the header row, the figures the row
 * under it, and the basis each figure rests on the row under that -- because a
 * headline number gets quoted without its denominator otherwise. Three rows
 * rather than three columns keeps the numbers side by side, which is how they
 * are compared.
 *
 * The wrapper scrolls sideways rather than reflowing: six columns need about
 * 780px, and below that the honest options are a scroll or a lie about how much
 * fits.
 */
function statTable(stats) {
  const row = (cls, cell) => el("tr", { class: cls }, stats.map(cell));
  return el("div", { class: "table-wrap stats-wrap" }, [
    el("table", { class: "stats" }, [
      el("thead", {}, [
        row(null, (s) => el("th", { scope: "col", text: s.label })),
      ]),
      el("tbody", {}, [
        row("values", (s) => el("td", { class: "value", text: s.value })),
        row("bases", (s) => el("td", { class: "basis", text: s.basis || "" })),
      ]),
    ]),
  ]);
}
...
async function renderOverview(host) {
  ...
  // The passed-every-rule count and its share were a hero card and a tile
  // saying the same thing twice; as neighbouring columns the repetition is
  // obvious, so they are one column with the share as its basis line.
  const headline = statTable([
    {
      label: "Rows in the file",
      value: fmtInt(h.trips),
      basis: "every line accounted for",
    },
    {
      label: "Passed all 14 quality rules",
      value: fmtInt(h.clean_trips),
      // Terser than the old hero caption: this line is now one of six sitting
      // in a 200px column, not a sentence with the width of the page.
      basis:
        `${fmtPct(cleanPct)} of rows. ` +
        `${fmtInt(h.trips - h.clean_trips)} flagged, not deleted.`,
    },
    {
      label: "Typical trip",
      value: fmtNum(h.typical_miles, 2) + " mi",
      basis: fmtNum(h.typical_minutes, 1) + " min, usable rows only",
    },
    {
      label: "Typical fare",
      value: fmtMoney(h.typical_fare),
      basis: "rows with a usable fare",
    },
    {
      label: "Total recorded revenue",
      value: "$" + fmtCompact(h.total_revenue),
      basis: "all rows, as recorded",
    },
    {
      label: "Pickup zones seen",
      value: fmtInt(h.pickup_zones),
      basis: "of 265 in the lookup",
    },
  ]);
  ...
  for (const metric of CONTRAST_METRICS) {
    ...
    columnChart(fig.plot, {
      ...
      series: [{
        key: "value",
        label: metric.title,
        color: (_row, i) => (i === 0 ? SERIES_2 : SERIES_1),
      }],
      directLabels: true,
      valueFmt: metric.fmt,
      height: 170,
...
/**
 * A labelled block of source, verbatim.
 *
 * `text` rather than `html`, so the body cannot inject markup into the page
 * however it was rendered server-side.
 */
function codeBlock(label, text) {
  return el("div", { class: "code-block" }, [
    el("div", { class: "code-label", text: label }),
    el("pre", {}, [el("code", { text: text })]),
  ]);
}

/**
 * The bench tab's fourth comparison, in full.
 *
 * Two cards rather than one. The left is the *definition* -- the Python body as
 * deployed, plus how well its constants fit -- and the right is the *result*.
 * They are separate because they answer separate questions: "is this really
 * running Python inside the database" and "what did it find". Printing the source
 * instead of describing it is deliberate: "the IRIS side is a Python UDF" is a
 * claim, and the body is the evidence for it.
 *
 * The constants are not a published rate card. They were fitted by least squares
 * to the loaded trips when the function was deployed, which is why the fit
 * quality is shown next to them -- a mean gap is not readable without knowing how
 * tight the model it is a gap from actually is.
 */
function benchUdfCards(udf) {
  const t = udf.tariff;
  const flat = Object.entries(t.flat_fares || {})
    .map(([code, fare]) => `rate ${code} → ${fmtMoney(fare)}`)
    .join(", ");
  // Read off the result rather than stated: the function is called once per trip
  // in the population, so the row counts it returned *are* the invocation count.
  const calls = udf.gap_by_rate_code.reduce((n, r) => n + Number(r.trips || 0), 0);

  const definition = el("figure", { class: "card" }, [
    el("figcaption", {}, [
      el("div", { class: "title", text: "The Python that ran inside IRIS" }),
      el("div", {
        class: "subtitle",
        text:
          "One statement compiles this body into the IRIS process, where SQL can " +
          "then call it per row like any built-in function",
      }),
    ]),
    codeBlock("Deployed verbatim, at the end of the pipeline", udf.create_sql),
    statTable([
      {
        label: "Fitted on",
        value: fmtInt(t.rows_fitted),
        basis: "clean, plainly-metered trips",
      },
      {
        label: "Median error",
        value: fmtMoney(t.median_error),
        basis: "half the trips are closer than this",
      },
      {
        label: "95th-percentile error",
        value: fmtMoney(t.p95_error),
        basis: `well inside the ${fmtMoney(udf.off_tariff_dollars)} threshold`,
      },
      {
        label: "Flat fares",
        value: flat || "—",
        basis: "read off the data as the modal fare",
      },
    ]),
    el("div", { class: "fig-foot" }, [
      el("span", {
        class: "note-text",
        text:
          "The pandas arm of the comparison executes this same rendered text, so " +
          "the two arms cannot disagree about what the formula says — only about " +
          "where it ran.",
      }),
    ]),
  ]);

  const result = el("figure", { class: "card" }, [
    el("figcaption", {}, [
      el("div", { class: "title", text: "What it found" }),
      el("div", {
        class: "subtitle",
        text:
          `Tariff: ${fmtMoney(t.base)} + ${fmtMoney(t.per_mile)}/mile + ` +
          `${fmtMoney(t.per_minute)}/minute. "Off tariff" is a fare more than ` +
          `${fmtMoney(udf.off_tariff_dollars)} from it.`,
      }),
    ]),
    codeBlock(
      `${fmtInt(calls)} calls to ${udf.function} inside the server, ` +
        `${udf.gap_by_rate_code.length} rows back`,
      udf.query
    ),
    dataTable(udf.gap_by_rate_code, {
      format: {
        trips: (v) => fmtInt(v),
        mean_gap: (v) => fmtMoney(v),
        off_tariff: (v) => fmtInt(v),
        pct_off_tariff: (v) => fmtNum(v, 2) + "%",
      },
    }),
    el("div", { class: "fig-foot" }, [
      el("span", {
        class: "note-text",
        text:
          "Rate codes 3–6 and 99 are out of scope rather than unmodelled: a " +
          "negotiated fare has no tariff to deviate from, so the function returns " +
          "NULL and the query excludes them.",
      }),
    ]),
  ]);
...
async function renderBench(host) {
  ...
  host.replaceChildren(
    el("p", { class: "status-msg", text: "Running both arms of each comparison…" })
  );
  ...
  // "Aggregated in IRIS" would be wrong for the fare_vs_tariff row, whose
  // server-side arm is a Python function called per trip rather than a GROUP BY.
  // The legend and the table's arm column say which kind each row is, because the
  // weaker claim -- the work moves to the data whether or not it is expressible as
  // SQL -- is the more useful one, and it is invisible if every row reads "SQL".
  const armLabel = (v) => (v ? "Python UDF" : "SQL GROUP BY");
  const anyUdf = agg.some((r) => r.uses_udf);
  ...
  const timeFig = figure({
    ...
    full: true,
    legend: [
      { label: "Computed in IRIS", color: SERIES_1 },
      { label: "Pulled into pandas", color: SERIES_2 },
    ],
    tableRows: agg,
    tableOptions: {
      columns: [
    ...
    footNote:
      (allAgree
        ? "Every pair of answers was asserted equal before timing was reported"
        : "WARNING: at least one pair of answers disagreed — see the table") +
      (anyUdf
        ? " · one row's IRIS-side arm is a Python function, not SQL — see below"
        : ""),
  });
  columnChart(timeFig.plot, {
    ...
    xKey: "comparison",
    series: [
      { key: "iris_seconds", label: "IRIS", color: SERIES_1 },
      { key: "python_seconds", label: "pandas", color: SERIES_2 },
    ],
    height: 210,
    labelEvery: 1,
    ...
    tooltipRows: (r) => [
      ["IRIS-side", fmtNum(r.iris_seconds, 3) + "s"],
      ["Python-side", fmtNum(r.python_seconds, 3) + "s"],
      ["Speed-up", fmtNum(r.speedup, 1) + "×"],
      ["Same answer", r.same_answer ? "yes" : "NO"],
      ["IRIS-side arm", armLabel(r.uses_udf)],
    ],
  });
  ...
  const movedCard = el("figure", { class: "card full" }, [
    ...
    dataTable(agg, {
      ...
      format: { uses_udf: armLabel, rows_ratio: (v) => fmtInt(v) + "×" },
    }),
  ]);
  ...
  // Only rendered when the payload carries a fitted tariff. It will not if the
  // pipeline has not been run, and a card explaining a model that does not exist
  // is worse than no card.
  const udfCards = data.udf && data.udf.tariff ? benchUdfCards(data.udf) : null;
  ...
  host.replaceChildren(
    el("p", {
      ...
      html:
        ...
        (allAgree
          ? `Both sides returned identical answers to all ${agg.length} questions.`
    ...
    udfCards ? el("div", { class: "grid" }, udfCards) : null,
