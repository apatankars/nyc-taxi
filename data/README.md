...
section of this README is [the friction log](#the-friction-log) — twenty-two
...
- [The stretch goal: IRIS-side vs Python-side](#the-stretch-goal-iris-side-vs-python-side)
...
| **Stretch:** filter/aggregate in IRIS vs pull into Python | `bench.py`, measured with correctness assertions; `udf.py` covers the question that is *not* expressible as `GROUP BY` |
...
- **Where the work is not an aggregate, the Python goes to the data instead.**
  `udf.py` compiles one function into IRIS with `CREATE FUNCTION ... LANGUAGE
  PYTHON`, so a per-row fare decision still runs server-side and still returns two
  rows. The principle above is really about *where rows are processed*, not about
  SQL: this is the case that separates the two readings.
...
| 7. deploy the UDF | 1.59s | fit the fare tariff, compile it into IRIS as Python |
| **Total** | **~92s** | empty database → indexed, flagged, queryable |
...
This is the substance of the exercise. Twenty-two items, grouped, each with the
real error text where there was one.
...
on `process`/`document` so neither fires. The palette (categorical slots 1–2 plus
the blue sequential ramp) **passes all five computed checks in both light and
dark mode** under the strictest all-pairs setting — worst CVD ΔE 24.7 light /
26.8 dark against a target of 8.0, and ≥3:1 contrast on both surfaces.
...
$ osascript -l JavaScript tools/render_check.js
  overview    278 nodes   5 svg    18 marks    6 bars   1 tables
  ...
  bench       184 nodes   1 svg     8 marks    3 bars   3 tables  2 source blocks
...
#### 21. `CREATE FUNCTION` rejects an underscore in a parameter name

Writing the fare UDF, the first version would not compile:

```sql
CREATE OR REPLACE FUNCTION Taxi.ExpectedFare(rate_code INTEGER, miles DOUBLE, minutes DOUBLE)
```
```
ERROR #5488: Invalid method formalspec format
Taxi.funcExpectedFare.ExpectedFare, expected <identifier> [OFFSET=10]
```

Nothing in that message is the parameter list. `Taxi.funcExpectedFare` is a class
IRIS generates behind the function, `formalspec` is its internal term for a
signature, and `ExpectedFare` is a method on the generated class — three names
that do not appear in the statement I wrote. So the natural reading is "something
about my function is malformed", and the search space is the whole statement.

`OFFSET=10` looks like the one usable hint, and it is worse than noise, because it
is an offset into the *generated* formalspec rather than into the statement I
wrote. I measured what it actually tracks:

| Signature | Offset | Where that lands in my text |
|---|---|---|
| `rate_code INTEGER, miles DOUBLE` | 10 | just past `rate_code` — plausible |
| `a_b INTEGER, miles DOUBLE` | 4 | just past `a_b` — plausible |
| `longer_name INTEGER, miles DOUBLE` | 12 | just past `longer_name` — plausible |
| `miles DOUBLE, rate_code INTEGER` | 32 | one *past the end* of the signature |
| `ratecode INTEGER, a_b DOUBLE` | 68 | past the end by 40 characters |

So it is only coincidentally useful when the *first* parameter is the offending
one, and it points off the end of the string when the offender is any later one.
Note also that the offset lands *after* the identifier, not on the underscore, so
even the plausible-looking cases do not name the character at fault.

I found the actual rule by bisecting candidate signatures rather than by reading
the error:
...
Underscores are fine in the function's *own* name (`Taxi.Expected_Fare` compiles)
and in every column name in this project. It is only the parameter list. So
`udf.py` is the one file here that uses camelCase, with a comment saying why —
otherwise the next person reformats it to match the house style and it stops
compiling.

**Cost:** forty minutes, most of it spent looking at the function body.
**Fix for the guide:** name the offending parameter in the error, or say
`identifiers in a formalspec must be alphanumeric` — either would have made this
a ten-second fix.

#### 22. The equality assertion caught two bugs the timings would have hidden

`bench.py` asserts the IRIS-side and Python-side answers are equal before it
reports either timing. On the UDF comparison it failed:

```
off_tariff: 24890 (IRIS) vs 24887 (pandas)
```

Three trips out of 702,103. Both arms run the *same rendered function text*, so
the formula could not be the difference — which is exactly why the failure was
worth having: it could only be the runtime. **IRIS evaluates the arithmetic in
fixed-point decimal and pandas in float64**, and a handful of trips sit exactly on
the `> $5.00` threshold, where a float64 gap of `4.999999999999999` falls on the
other side of the comparison from a decimal `5.00`. Neither is wrong. Rounding to
cents *before* the threshold test puts both arms on values that are exact in
either representation.

Chasing that turned up a second, worse bug in my own SQL. I had written the
threshold the obvious way:

```sql
-- calls the function TWICE per row
SELECT ROUND(AVG(ABS(FareAmount - Taxi.ExpectedFare(...))), 2),
       SUM(CASE WHEN ABS(FareAmount - Taxi.ExpectedFare(...)) > 5.0 THEN 1 ELSE 0 END)
```

That is 1.4 million Python invocations for 702,103 rows, and it inflates the
measured cost of the UDF by roughly 2× — a benchmark quietly arguing against the
thing it was built to demonstrate. Isolating the per-row gap in a derived table
so the function is called once fixed both the count and the timing.

**The general point:** the assertion was written as a correctness guard, and both
bugs it caught were *measurement* bugs. A benchmark without one does not fail, it
just reports something else. This is also the argument for rendering the function
body once and running the same text in both places: if the two arms could each
have their own copy of the formula, a 3-row disagreement is indistinguishable
from a typo, and the temptation is to shrug at it.

**Cost:** ninety minutes, and worth every one of them.
**Fix for the guide:** the stretch goal should say *assert the answers match*, not
just *compare the timings*. Mine only found these because it did.
...
## The stretch goal: IRIS-side vs Python-side
...
Each question is answered twice — once inside IRIS, versus `SELECT` the raw
columns and reduce in pandas. **Both answers are asserted equal with
`pd.testing.assert_frame_equal` before either timing is reported**, because a
faster wrong answer is not a result. All four agree.

| Question | IRIS | pandas | Speed-up | Rows moved | Ratio |
|---|---|---|---|---|---|
| Revenue by pickup borough | 0.030s | 0.697s | **23.0×** | 8 vs 787,060 | 98,382× |
| Trips & distance by hour | 0.026s | 0.650s | **25.4×** | 24 vs 787,060 | 32,794× |
| Distribution of issue counts | 0.010s | 0.314s | **30.9×** | 6 vs 787,060 | 131,177× |
| Fare vs tariff, per rate code † | 0.104s | 1.538s | **14.8×** | 2 vs 702,103 | 351,052× |

*(median of 3 runs. † is the one whose IRIS side is not SQL — see below.)*

### When the work is not an aggregate

The first three questions are `GROUP BY`, which makes "push the work down" and
"rewrite it as SQL" look like the same sentence. They are not, and the fourth
question is there to separate them.

*Does this trip's metered fare match the tariff it should have been charged on?*
is a per-row decision over a rate-code table. SQL can express it as a nested
`CASE`, and the expression is harder to read than the rule it encodes. So its
IRIS-side arm is a **Python function compiled into the database**:

```sql
CREATE OR REPLACE FUNCTION Taxi.ExpectedFare(rateCode INTEGER, miles DOUBLE, minutes DOUBLE)
RETURNS DOUBLE
LANGUAGE PYTHON
{ ... }
```

702,103 Python calls then happen server-side and two rows come back. The useful
version of the claim turns out to be the weaker one: **the work moves to the data
whether or not it can be expressed as a query.**

Two things keep this from being a demo.

**One definition, two runtimes.** `udf.py` renders the function body *once*.
`create_sql()` wraps that text for IRIS; `python_callable()` `exec`s the identical
text in this process for the pandas arm. Neither is a transcription of the other,
so the equality assertion is testing the plumbing rather than two hand-maintained
copies of one formula. It earned its keep immediately — see friction #22.

**The tariff is measured, not asserted.** A hard-coded rate card would put a
number in the repo that nobody here can verify. `calibrate()` fits it by least
squares to the clean metered trips ($2.28 + $2.65/mile + $0.49/minute, median
error $0.38, p95 $1.87) and reads the flat fares off the data as the modal fare
per rate code ($70.00 on rate 2, from 2,088 of its 2,163 trips). My guessed
published rate card was six times worse.

| Rate code | Trips | Mean gap | More than $5 off | % |
|---|---|---|---|---|
| 1 (metered) | 699,940 | $3.28 | 24,885 | 3.56% |
| 2 (JFK flat) | 2,163 | $3.75 | 75 | 3.47% |

Rate codes 3–6 and 99 are out of scope rather than unmodelled: a negotiated fare
has no tariff to deviate from, so the function returns `NULL` and the query
excludes them. The dashboard's last tab prints the deployed function verbatim
next to these numbers, because "the IRIS side is a Python UDF" is a claim and the
source is the evidence for it.
...
**On speed:** IRIS-side wins, by 23–31× on the three aggregations and 14.8× on
the UDF question — lower there because a Python call per row is genuinely more
work than a `GROUP BY`, and the honest comparison keeps both arms doing the same
per-row work rather than vectorising one of them. But the ingest table is
...
src/taxi/
  ...
  quality.py    799   14 rules as Python data, compiled to two UPDATEs;
                      each rule declares the measures it invalidates
  analytics.py  572   the user-facing workflows; IRIS aggregates, pandas labels
  udf.py        366   one Python function, rendered once and run in two places:
                      compiled into IRIS, and exec'd here for the comparison
  bench.py      464   the stretch goal, with correctness assertions
  cli.py        205   info / pipeline / profile / quality / analyze / bench
  web.py        387   JSON endpoints over analytics + quality; serves the page

src/taxi/static/
  index.html    115   the whole page skeleton
  app.js       1844   hand-rolled SVG charts; no framework, no build, no CDN
  styles.css    689   design tokens + layout; every colour is a named token
  validate_palette.js   the colour validator, vendored so it can run in-browser

tools/
  render_check.js  368   executes every panel headlessly (friction #20)
