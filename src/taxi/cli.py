...
from . import analytics, db, load, quality, schema, transform
...
def cmd_pipeline(args: argparse.Namespace) -> None:
    ...
    print("Stage 1/6: schema")
    schema.create_all()

    print("\nStage 2/6: bulk load (server-side LOAD DATA)")
    load.load_all()

    print("\nStage 3/6: cast, derive and enrich")
    counts = transform.cast_and_enrich()

    print("\nStage 4/6: quality rules")
    rule_summary = quality.apply_rules()

    print("\nStage 5/6: indexes")
    schema.create_indexes()

    print("\nStage 6/6: optimiser statistics")
    schema.tune_tables()
