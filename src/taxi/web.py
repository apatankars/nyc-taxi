...
def api_bench():
    ...
    pulls every trip across the driver, three times over, and the embedded arm
    scans them again inside the server. Cached once run, so the button can be
    pressed again during a demo without paying for it twice.

    ``embedded_note`` is None when the embedded Python arm ran, and carries the
    reason it did not when its columns come back null.
    """
    repeat = _int("repeat", 3, maximum=10)
    ingest_rows = _int("ingest_rows", 25_000, maximum=200_000)

    def produce():
        from . import bench  # imported lazily; nothing else here needs it
        ...
        return {
            ...
            "embedded_note": aggregation.attrs.get("embedded_note"),
